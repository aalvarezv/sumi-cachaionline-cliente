...
const InputSelectModulosContenido = React.forwardRef((props,ref) => {
    ...
    useEffect(() => {
        ...
        const listarContenidosModulo = async () => {
            try{
                const resp = await clienteAxios.get(`/api/contenidos/listar/${codigo_modulo}`)
    ...
    return (
        <Form.Control
            ...
            ref={ref}
        >
            <option key="0" value="0">{props.label ? props.label : 'SELECCIONE CONTENIDO DEL MÓDULO'}</option>
