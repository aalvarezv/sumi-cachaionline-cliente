...
const InputSelectModulosContenidoTema = React.forwardRef((props,ref) => {
    ...
    useEffect(() => {
        const listarModuloContenidoTemas = async () =>{
            try{
                const resp = await clienteAxios.get(`/api/temas/listar/${codigo_modulo_contenido}`)
    ...
    return (
        <Form.Control 
            ...
            ref={ref}
        >
            <option key="0" value="0">{props.label ? props.label : 'SELECCIONE TEMA DEL CONTENIDO'}</option>
